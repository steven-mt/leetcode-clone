    ...
    <div className="text-white z-40">
      ...
      >
        <div className="flex min-h-screen items-center justify-center px-4">
          ...
          <div
            className="my-8 inline-block min-w-full transform rounded-[13px] text-left
          ...
          >
            {/* setting header */}
            <div
              className="flex items-center border-b px-5 py-4 text-lg font-medium
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="flex cursor-pointer items-center rounded px-3 py-1.5
                        text-left focus:outline-none whitespace-nowrap bg bg-dark-fill-3
                        hover:bg-dark-fill-2 active:bg-dark-fill-3 w-full justify-between"
                      type="button"
                    >
                      {LSFontSize}
                      <BsChevronDown />
                    ...
                    {settings.dropdownIsOpen && (
                      <ul
                        className="absolute mt-1 max-h-56 overflow-auto rounded-lg
                          p-2 z-50 focus:outline-none shadow-lg w-full bg-dark-layer-1"
                        style={{
                          filter:
                      ...
                      >
                        {EDITOR_FONT_SIZES.map((fontSize, idx) => (
                          <SettingsListItem
                            key={idx}
                            fontSize={fontSize}
                            selectedOption={LSFontSize}
                            handleFontSizeChange={(fontSize) => {
                              setLSFontSize(fontSize);
...
const SettingsListItem: React.FC<SettingsListItemProps> = ({
  ...
  return (
    <li
      className="relative flex h-8 cursor-pointer select-none py-1.5 pl-2 text-label-2
